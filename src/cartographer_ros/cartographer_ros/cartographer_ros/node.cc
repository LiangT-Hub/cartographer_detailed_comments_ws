...
::ros::Subscriber SubscribeWithHandler(
  ...
  return node_handle->subscribe<MessageType>(
      ...
      // comment_L:使用boost::function构造回调函数,被subscribe注册?????
...
Node::Node(
  ...
  // comment_L:创建一个 ROS 发布者，
  // 该发布者将会在名为 kSubmapListTopic 的话题上发布 
  // ::cartographer_ros_msgs::SubmapList 类型的消息。
  // 通过使用 node_handle_.advertise 函数，程序向 ROS 核心注册了这个发布者，并指定了发布队列的大小 kLatestOnlyPublisherQueueSize
  // 发布SubmapList
  ...
  // comment_L:创建多个服务器：
  // 调用`node_handle_.advertiseService`创建一个服务器
  // 需要三个函数：服务名称、服务回调函数的指针、一个指向当前对象的指针（this），用于将成员函数作为回调函数
...
/**
 * @brief 新增一个位姿估计器，IMU与里程计的融合，作预测，匹配之前的先验
 * 
 * @param[in] trajectory_id 轨迹id
...
void Node::AddExtrapolator(const int trajectory_id,
  ...
  // comment_L: 完美转发 https://www.cnblogs.com/guxuanqing/p/11396511.html
...
void Node::AddSensorSamplers(const int trajectory_id,
  ...
  // comment_L :std::forward_as_tuple?????
