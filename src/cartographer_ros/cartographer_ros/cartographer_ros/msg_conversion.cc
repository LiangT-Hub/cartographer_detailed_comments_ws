...
std::unique_ptr<nav_msgs::OccupancyGrid> CreateOccupancyGridMsg(
  ...
  const int occ_grid_unknown = -1;
  const int occ_grid_free = 0;
  const int occ_grid_occupied = 100;
  ...
  for (int y = height - 1; y >= 0; --y) {
    for (int x = 0; x < width; ++x) {
      const uint32_t packed = pixel_data[y * width + x];  // const int occ_grid_unknown = -1;
  // const int occ_grid_free = 0;
  // const int occ_grid_occupied = 100;
